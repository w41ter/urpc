...
public:
    ...
    // Explicitly declare this overload as error to avoid copy_to(urpc::IOBuf*)
    ...
    bool equals(std::string_view) const;
...
inline bool operator==(const urpc::IOBuf& b, const std::string_view& s) {
    return b.equals(s);
}
inline bool operator==(const std::string_view& s, const urpc::IOBuf& b) {
    return b.equals(s);
}
inline bool operator!=(const urpc::IOBuf& b, const std::string_view& s) {
    return !b.equals(s);
...
inline bool operator==(const urpc::IOBuf& b1, const urpc::IOBuf& b2) {
    return b1.equals(b2);
}
inline bool operator!=(const urpc::IOBuf& b1, const urpc::IOBuf& b2) {
    return !b1.equals(b2);
...
class IOBufCutter {
public:
    explicit IOBufCutter(urpc::IOBuf* buf);
    ~IOBufCutter();

    // Cut off n bytes and APPEND to `out'
    // Returns bytes cut.
    size_t cutn(urpc::IOBuf* out, size_t n);
    size_t cutn(std::string* out, size_t n);
    size_t cutn(void* out, size_t n);
...
public:
    ...
    int append(std::string_view str);
...
public:
    explicit IOBufBytesIterator(const urpc::IOBuf& buf);
    ...
    size_t append_and_forward(urpc::IOBuf* buf, size_t n);
...
namespace std {
template <>
inline void swap(urpc::IOBuf& a, urpc::IOBuf& b) {
    return a.swap(b);
}
}  // namespace std
...
inline int IOBufAppender::append(std::string_view str) {
...
inline IOBufBytesIterator::IOBufBytesIterator(const urpc::IOBuf& buf)
    : _block_begin(NULL),
      _block_end(NULL),
      _block_count(0),
...
inline void IOBufBytesIterator::try_next_block() {
    ...
    auto s = _buf->backing_block(_block_count++);
    _block_begin = s.data();
