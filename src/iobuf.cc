...
inline iov_function get_preadv_func() {
    urpc::fd_guard fd(open("/dev/zero", O_RDONLY));
    if (fd < 0) {
...
inline iov_function get_pwritev_func() {
    urpc::fd_guard fd(open("/dev/null", O_WRONLY));
    if (fd < 0) {
...
IOBuf::Block* share_tls_block() {
    ...
    if (b) {
        ...
        std::atexit(remove_tls_block_chain);
    }
    if (!new_block) {
...
bool IOBuf::equals(const urpc::IOBuf& other) const {
...
IOBufCutter::IOBufCutter(urpc::IOBuf* buf)
...
size_t IOBufCutter::cutn(urpc::IOBuf* out, size_t n) {
    if (n == 0) {
        return 0;
    }
...
size_t IOBufBytesIterator::append_and_forward(urpc::IOBuf* buf, size_t n) {
    size_t nc = 0;
...
void* fast_memcpy(void* __restrict dest, const void* __restrict src, size_t n) {
    return urpc::iobuf::cp(dest, src, n);
}
